...
export function LeadsPage() {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-col gap-3 sm:flex-row"><Input placeholder="Search phone or social ID" aria-label="Search leads" value={search} onChange={e => { setSearch(e.target.value); setOffset(0) }} /><Select aria-label="Lead status" value={status} onChange={e => { setStatus(e.target.value); setOffset(0) }} options={['all', 'enquiry', 'converted'].map(value => ({ value, label: value.replace(/^./, c => c.toUpperCase()) }))} /></div>
    <ErrorNotice error={query.error} retry={() => void query.refetch()} />
    {query.isPending ? <PageSkeleton label="Loading leads…" variant="table" /> : query.data && <>
...
export function LeadDetail() {
  ...
  return <div className="max-w-4xl space-y-5">
    ...
    <ErrorNotice error={query.error} retry={() => void query.refetch()} />
    {query.isPending && <PageSkeleton label="Loading lead…" variant="form" />}
    ...
    <section className={card}><h3 className="mb-4 font-semibold">Enquiry history</h3><ErrorNotice error={history.error} retry={() => void history.refetch()} />
      {history.isPending && <PageSkeleton label="Loading enquiries…" variant="table" />}
