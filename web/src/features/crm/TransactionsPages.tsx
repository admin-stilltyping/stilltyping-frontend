import { PageSkeleton, InlineLoading } from '@/components/ui/LoadingState'
...
function OrderForm({ slug, initial, onDone }: { slug: string; initial: Recipient; onDone: (id: string) => void }) {
  ...
  return <form onSubmit={e => { e.preventDefault(); save.mutate() }} className="space-y-4"><fieldset className="space-y-4" disabled={save.isPending}>
    ...
    <section className="space-y-3"><h3 className="font-semibold">Products</h3><Input aria-label="Find products" placeholder="Search products" value={search} onChange={e => { setSearch(e.target.value); setOffset(0) }} /><ErrorNotice error={query.error} retry={() => void query.refetch()} />
      {query.isPending && <PageSkeleton label="Loading products…" variant="table" />}
...
export function OrderList() {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold">Orders</h2><Button onClick={() => setCreating(true)}>New order</Button></div>
    <Select aria-label="Order status" value={status} onChange={e => { setStatus(e.target.value); setOffset(0) }} options={['all', 'confirmed', 'fulfilled', 'cancelled'].map(value => ({ value, label: value.replace(/^./, c => c.toUpperCase()) }))} />
    <ErrorNotice error={query.error} retry={() => void query.refetch()} />{query.isPending && <PageSkeleton label="Loading orders…" variant="table" />}
...
export function OrderDetail() {
  ...
  return <div className="max-w-4xl space-y-5"><Link to="/orders" className={linkClass}>← Orders</Link><ErrorNotice error={query.error} retry={() => void query.refetch()} />{query.isPending && <PageSkeleton label="Loading order…" variant="form" />}
...
function AppointmentForm({ slug, initial, onDone }: { slug: string; initial: Recipient; onDone: (id: string) => void }) {
  ...
  return <form onSubmit={e => { e.preventDefault(); save.mutate() }} className="space-y-4"><fieldset className="space-y-4" disabled={save.isPending}>
    ...
    <Select label="Service" disabled={query.isPending || !!query.error} required value={serviceId} onChange={e => setServiceId(e.target.value)} options={[{ value: '', label: 'Select service…' }, ...services.map(s => ({ value: s.id, label: `${s.name} · ${s.duration_minutes} min · ${s.currency} ${s.price}` }))]} />
    {query.isPending && <InlineLoading label="Loading services…" />}
    {query.data?.total === 0 && <p className="text-sm text-gray-500">No active services found. Add a service before booking an appointment.</p>}
    {query.data && <Pager offset={offset} total={query.data.total} onChange={setOffset} />}
    ...
    <ErrorNotice error={save.error} /><Button type="submit" disabled={!serviceId || !scheduledAt || !!query.error} loading={save.isPending}>Book appointment</Button>
  </fieldset></form>
}
...
export function AppointmentList() {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold">Appointments</h2><Button onClick={() => setCreating(true)}>New appointment</Button></div>
    <Select aria-label="Appointment status" value={status} onChange={e => { setStatus(e.target.value); setOffset(0) }} options={['all', 'scheduled', 'confirmed', 'completed', 'cancelled', 'no_show'].map(value => ({ value, label: value.replace('_', ' ').replace(/^./, c => c.toUpperCase()) }))} />
    <ErrorNotice error={query.error} retry={() => void query.refetch()} />{query.isPending && <PageSkeleton label="Loading appointments…" variant="table" />}
...
export function AppointmentDetail() {
  ...
  return <div className="max-w-3xl space-y-5"><Link to="/appointments" className={linkClass}>← Appointments</Link><ErrorNotice error={query.error} retry={() => void query.refetch()} />{query.isPending && <PageSkeleton label="Loading appointment…" variant="form" />}
