...
export function AiUsagePage() {
  ...
  const cacheIncomplete = !!summary?.cache_incomplete_replies
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <dl className="flex flex-wrap gap-x-8 gap-y-3">
              <div><dt className="text-xs text-slate-500">Cached input</dt><dd className="mt-1 font-semibold tabular-nums text-emerald-700">{cacheTokens(summary!.cached_input_tokens, !cacheIncomplete)}</dd></div>
              <div><dt className="text-xs text-slate-500">Uncached input</dt><dd className="mt-1 font-semibold tabular-nums text-slate-900">{cacheTokens(summary!.uncached_input_tokens, !cacheIncomplete)}</dd></div>
            </dl>
          </div>
          {cacheIncomplete && <p className="mt-3 text-xs text-slate-500">Cache counts are unavailable for some replies, including activity before cache tracking began. Reported portions are shown with “+”.</p>}
        </div>
      </>}
